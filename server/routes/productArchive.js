...
import ArchiveDetailsModel from "../models/Inventory/StorageArchive.js";
...
router.get("/", async (req, res) => {
  try {
    const products = await ArchiveDetailsModel.find();
    res.json(products);
  } catch (error) {
    console.error(error);
...
router.post("/", async (req, res) => {
  try {
    const newProduct = new ArchiveDetailsModel(req.body);
...
router.post("/bulk", async (req, res) => {
  try {
    ...
    for (const product of productsWithId) {
      const existingProduct = await ArchiveDetailsModel.findOne({
    ...
    // Execute bulk write
    const result = await ArchiveDetailsModel.bulkWrite(bulkOps);
